...
def test_isolated_operator__new__():
    actual = IsolatedOperator(task_id="foo", image="foo", operator=IsolatedOperator)
    assert isinstance(actual, IsolatedKubernetesPodOperator), "isolation_type defaults to kubernetes"

    actual = IsolatedOperator(task_id="foo", isolation_type="kubernetes", image="foo", operator=IsolatedOperator)
    assert isinstance(
    ...
    with pytest.raises(NotImplementedError):
        ...
        IsolatedOperator(task_id="foo", isolation_type="something_else", image="foo", operator=IsolatedOperator)
